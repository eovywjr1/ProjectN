...
void UPNGameplayAbility_AttackHitCheck::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	if (const UPNWeaponAttributeSet* WeaponAttributeSet = GetAbilitySystemComponentFromActorInfo()->GetSet<UPNWeaponAttributeSet>())
	{
		...
		if (TargetActorClass)
		{
			...
			AttackTraceTask->OnComplete.AddUObject(this, &ThisClass::OnTraceResultCallback);
			AttackTraceTask->ReadyForActivation();
		}
	}
}

void UPNGameplayAbility_AttackHitCheck::OnTraceResultCallback(const TArray<TWeakObjectPtr<AActor>>& FoundActors)
{
